...
* Exercise to make, grow, and print a list of doubles, using preprocessor functions. 
*/
...
#define CHECK_CAP(type, name) if (name##_len >= name##_cap) { name##_cap *= 2; name = (type *)realloc(name, name##_cap * sizeof(type)); }
#define SIZE(name) name##_len
#define FOR(var, limit) for(int var = 0; var < (limit); var++)
#define SWAP(type, a, b) { type tmp = a; a = b; b = tmp; }
/**
  * This is the main method of the whole program, handling any errors, otherwise exiting successfully. (Partially completed)
