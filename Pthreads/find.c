...
// This program uses processes in a for-loop to search files for a given word (string),
// printing out found lines by file in the findWord() function.
// Main functionality in main() for dealing with processes and Console I/O.

// All libraries used are imported below, to deal with processes, strings, and Console I/O.
#include <unistd.h>
#include <sys/wait.h>
#include <stdio.h>
#include <stdlib.h>
...
// @return void Returns nothing, all output is to stdout via printf().
void findWord(void * fileName);

// Global variable to store the word to search for in.
char * word;

// This function contains the main functionality of the program, dealing with Console I/O and process forking.
// This function takes in the number of command line arguments and their contents, passes those to findWord() 
// via (char *) word and (void *) fileName, and forks/executes/terminates (via wait(NULL)) a process for each file passed.
...
int main(int argc, char * argv[])
{
   ...
   // For-loop to fork/execute/terminate processes based on the number of file(s) (parameters 3-7 in (char *) argv[], with parameters 4-7 being optional)
   for(int i = 2; i < argc; i++) {
    ...
    if(fork() == 0) {
       // If successfully forked, execute findWord() for each file.
       findWord(argv[i]); 
     }
...
void findWord(void * fileName)
{
   ...
   // After the last line is found, print a new line, free the dynamic storage for currLine, close the searchFile, and exit the process successfully.
   printf("\n");
   free(currLine);
   fclose(searchFile);
   exit(EXIT_SUCCESS);
...
// Note: Ran on Unix/Linux box via MobaXterm/SSH; Compilation command: gcc -Wall -std=c99 -g find.c -o find; Test command ./find <word> <files>
